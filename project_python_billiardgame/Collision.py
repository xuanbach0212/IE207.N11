...
def check_collision_with_other_ball(x, y, ball1):
    for b in balls: 
        if not ball1.collision_monitor[balls.index(b)] and not b.potted: # update
            if b.x != x and b.y != y:
...
def check_collision_with_other_ball_9(x, y, ball1):
    for b in balls_9: # update
        if not ball1.collision_monitor[balls_9.index(b)] and not b.potted: # update
            if b.x != x and b.y != y:
                if distance_between_points(x, y, b.x, b.y) <= 35:
                    return b
